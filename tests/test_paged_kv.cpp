...
int main() {
    ...
    for (size_t t = 0; t < T; ++t) {
        ...
        for (size_t j = 0; j < C; ++j) {
            k(0, j) = (float)t * 0.1f + j;
            v(0, j) = (float)t - (float)j * 0.5f;
        }
    ...
    for (size_t l = 0; l < L; ++l) {
        ...
        for (size_t i = 0; i < ka.data.size(); ++i)
            md = std::max(md, std::fabs(ka.data[i] - kb.data[i]));
        for (size_t i = 0; i < va.data.size(); ++i)
            md = std::max(md, std::fabs(va.data[i] - vb.data[i]));
    }
