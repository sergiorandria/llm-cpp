...
int main() {
    // block codec roundtrips
    {
        ...
        assert(md < 0.02f);
        float s4; uint8_t p[16]; float z[32];
    ...
    llm::Config cfg;
    cfg.vocab_size = 32; cfg.n_layers = 1; cfg.n_heads = 2; cfg.n_embd = 32; cfg.block_size = 16;
    ...
    for (int qt : {8, 4}) {
        ...
        for (auto p : m2.parameters()) for (auto& v : p->data) v += 1.0f;  // poison m2
