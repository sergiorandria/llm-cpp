...
int main() {
    ...
    float md = 0;
    for (size_t i = 0; i < y_sum.data.size(); ++i) md = std::max(md, std::fabs(y_sum.data[i]-y_merged.data[i]));
    ...
    llm::Config cfg;
    cfg.vocab_size = 8; cfg.n_layers = 1; cfg.n_heads = 2; cfg.n_embd = 4; cfg.block_size = 8;
