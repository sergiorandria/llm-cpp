...
int main() {
    ...
    x(1,0)=-1; x(1,1)=0; x(1,2)=1; x(1,3)=2;
    ...
    for (size_t i=0;i<2;++i) for (size_t j=0;j<4;++j) {
        ...
        float num=0; for (auto v: yp.data) num+=v; float nm=0; for (auto v: ym.data) nm+=v;
    ...
    for (size_t j=0;j<4;++j) {
        ...
        wp.data[j]+=eps; wm.data[j]-=eps;
        auto yp=x.rmsnorm(&wp), ym=x.rmsnorm(&wm);
        float sp=0, sm=0; for (auto v: yp.data) sp+=v; for (auto v: ym.data) sm+=v;
