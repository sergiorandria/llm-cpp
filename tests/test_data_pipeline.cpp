...
int main() {
    // B15: mmap vs ifstream equivalence
    {
        ...
        { std::ofstream o("/tmp/big.txt", std::ios::binary); o << big; }
    ...
    // B17: shuffle covers all tokens once, deterministic per seed
    {
        ...
        std::vector<int> a, b;
        while (l1.has_next()) { auto x = l1.next_batch(); a.insert(a.end(), x[0].begin(), x[0].end()); }
        while (l2.has_next()) { auto x = l2.next_batch(); b.insert(b.end(), x[0].begin(), x[0].end()); }
        ...
        std::vector<int> c;
        while (l4.has_next()) { auto x = l4.next_batch(); c.insert(c.end(), x[0].begin(), x[0].end()); }
