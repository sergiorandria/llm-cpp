...
int main() {
    ...
    std::string r = http_post(port, "/v1/completions",
    ...
    llm::HttpResponse d =
        srv.dispatch("POST", "/v1/completions", "{\"prompt\":\"Hi\",\"temperature\":0}");
    assert(d.status == 200);
    // chat completions body is strict-parseable UTF-8 JSON
    llm::HttpResponse ch =
        srv.dispatch("POST", "/v1/chat/completions",
