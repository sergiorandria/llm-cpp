...
int main() {
    ...
    for (auto& v : Wq.data) v = 0.1f;
    for (auto& v : Wk.data) v = -0.07f;
    for (auto& v : Wv.data) v = 0.05f;
    ...
    for (size_t i = 0; i < Wk1.data.size(); ++i) {
        Wk1.data[i] = Wk.data[i];
        Wv1.data[i] = Wv.data[i];
    }
    ...
    float md = 0;
    for (size_t i = 0; i < ymqa.data.size(); ++i)
        md = std::max(md, std::fabs(ymqa.data[i] - ygqa1.data[i]));
