...
int main() {
    ...
    float err2 = llm::test::max_fd_error(x, [&](const Tensor& xp) { return xp.layernorm(&gamma, nullptr); }, lg.grad_x);
    ...
    float err3 = llm::test::max_fd_error(x, [&](const Tensor& xp) { return xp.rmsnorm(&w); }, rg.grad_x);
