...
int main() {
    llm::Config ct, cs;
    ct.vocab_size = 16; ct.n_layers = 2; ct.n_heads = 2; ct.n_embd = 8; ct.block_size = 8;
    cs = ct; cs.n_layers = 1;
    ...
    for (size_t i = 0; i < batches.size(); ++i) {
        ...
        float md = 0;
        for (size_t j = 0; j < live.data.size(); ++j) md = std::max(md, std::fabs(live.data[j]-cached[i].data[j]));
        assert(md == 0.0f);
    }
    // cached KL grads match live within 1e-6
    for (size_t i = 0; i < batches.size(); ++i) {
        llm::GPT s1(cs), s2(cs);
        for (size_t k = 0; k < s1.parameters().size(); ++k) s2.parameters()[k]->data = s1.parameters()[k]->data;
