...
int main() {
    ...
    for (int i = 0; i < 3; ++i) {
        llm::HttpResponse r = srv.dispatch("POST", "/v1/completions",
    ...
    llm::HttpResponse o = srv.dispatch("POST", "/v1/completions",
        "{\"prompt\":\"" + big + "\",\"max_tokens\":50,\"temperature\":0}");
    assert(o.status == 400);
    // H75: validation table (status + message), calls hoisted
    struct Case { const char* body; int want; const char* msg; };
    Case cases[] = {
    ...
    llm::HttpResponse a1 = srv2.dispatch("POST", "/v1/completions", "{\"prompt\":\"Same\",\"max_tokens\":1,\"temperature\":0}");
    llm::HttpResponse a2 = srv2.dispatch("POST", "/v1/completions", "{\"prompt\":\"Same\",\"max_tokens\":1,\"temperature\":0}");
    assert(a1.status == 200 && a2.status == 200);
    // stable hash: last two audit lines carry the same prompt_hash
    std::vector<std::string> all;
    { std::ifstream af2("/tmp/audit.log"); std::string x; while (std::getline(af2, x)) all.push_back(x); }
