...
#include <cmath>
namespace llm {
Tensor flash_attention(const Tensor& Q, const Tensor& K, const Tensor& V, float scale, bool causal,
                       size_t block_size) {
    size_t T = Q.shape[0];
    size_t D = Q.shape[1];
...
Tensor flash_attention_incremental(const Tensor& Q, const Tensor& K, const Tensor& V, float scale,
                                   size_t block_size) {
    assert(Q.shape[0] == 1);
    ...
    for (size_t kb = 0; kb < K_len; kb += block_size) {
        ...
        for (size_t j = kb; j < ke; ++j) {
            float s = 0;
            for (size_t d = 0; d < D; ++d) s += Q(0, d) * K(j, d);
            s *= scale;
            maxv = std::max(maxv, s);
        }
    }
...
Tensor flash_attention_full(const Tensor& Q, const Tensor& K, const Tensor& V, float scale,
                            bool causal, size_t block_size) {
    size_t T = Q.shape[0];
    size_t D = Q.shape[1];
    size_t N = K.shape[0];
    Tensor out({T, D}, 0.0f);
