...
Config load_config(const std::string& path) {
    ...
    auto get = [&](const std::string& key, size_t def) {
        ...
        p = content.find(":", p);
        if (p == std::string::npos) return def;
        return (size_t)std::stoi(content.substr(p + 1));
    };
...
bool validate_config_verbose(const Config& cfg, std::string& err) {
    if (cfg.n_embd % cfg.n_heads != 0) {
        err = "n_heads (" + std::to_string(cfg.n_heads) + ") must divide n_embd (" +
              std::to_string(cfg.n_embd) + ")";
        return false;
    }
    if (cfg.vocab_size == 0) {
        err = "vocab_size must be > 0";
        return false;
    }
    if (cfg.n_layers == 0) {
        err = "n_layers must be > 0";
        return false;
    }
    // C24: ALiBi and RoPE are mutually exclusive position encodings
    if (cfg.use_alibi && cfg.pos_encoding == PosEncoding::RoPE) {
        err = "use_alibi and RoPE are mutually exclusive (pick one position encoding)";
        return false;
    }
    if (cfg.rope_scaling < 1.0f) {
        err = "rope_scaling must be >= 1.0";
        return false;
    }
    if (cfg.global_every == 0) {
        err = "global_every must be > 0";
        return false;
    }
    err.clear();
    return true;
