...
std::vector<int> beam_search(const GPT& model, const std::vector<int>& prompt,
                             size_t max_new_tokens, size_t beam_width) {
    BeamConfig c;
    c.beam_width = beam_width;
    return beam_search_cfg(model, prompt, max_new_tokens, c);
}

std::vector<int> beam_search_cfg(const GPT& model, const std::vector<int>& prompt,
                                 size_t max_new_tokens, BeamConfig cfg) {
    ...
    for (size_t step = 0; step < max_new_tokens; ++step) {
        ...
        std::sort(
            candidates.begin(), candidates.end(), [&](const BeamState& a, const BeamState& b) {
                return norm_score(a.score, a.tokens.size()) > norm_score(b.score, b.tokens.size());
            });
    ...
    auto best =
        std::max_element(beams.begin(), beams.end(), [&](const BeamState& a, const BeamState& b) {
            return norm_score(a.score, a.tokens.size()) < norm_score(b.score, b.tokens.size());
        });
    return best->tokens;
}
