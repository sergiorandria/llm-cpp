...
int sample_top_k_seeded(const std::vector<float>& logits, int k, float temp, uint64_t seed) {
    ...
    for (auto& v : tmp) v /= temp;
    std::partial_sort(idx.begin(), idx.begin() + k, idx.end(),
                      [&](int a, int b) { return tmp[a] > tmp[b]; });
    std::vector<float> topk(k);
    for (int i = 0; i < k; ++i) topk[i] = tmp[idx[i]];
...
int sample_top_p_seeded(const std::vector<float>& logits, float p, float temp, uint64_t seed) {
    ...
    filt.reserve(last + 1);
    for (int i = 0; i <= last; ++i) filt.push_back(l[idx[i]]);
    float s = 0;
    for (auto v : filt) s += v;
    for (auto& v : filt) v /= s;
    std::mt19937 rng((uint32_t)seed);
    std::discrete_distribution<int> dist(filt.begin(), filt.end());
    return idx[dist(rng)];
}
...
std::vector<float> apply_repetition_penalty(const std::vector<float>& logits,
                                            const std::vector<int>& gen, float penalty) {
    std::vector<float> out = logits;
    for (int id : gen)
        if (id >= 0 && id < (int)out.size()) {
            if (out[id] > 0)
                out[id] /= penalty;
            else
                out[id] *= penalty;
...
std::vector<float> apply_freq_presence(const std::vector<float>& logits,
                                       const std::vector<int>& gen, float freq_penalty,
                                       float pres_penalty) {
    std::vector<float> out = logits;
    std::unordered_map<int, int> counts;
    for (int id : gen)
        if (id >= 0 && id < (int)out.size()) counts[id]++;
    for (auto& kv : counts) {
...
std::vector<float> apply_logit_bias(const std::vector<float>& logits,
    ...
    std::vector<float> out = logits;
    for (auto& kv : bias)
        if (kv.first >= 0 && kv.first < (int)out.size()) out[kv.first] += kv.second;
    return out;
}
// E44
