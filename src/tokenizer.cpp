...
std::vector<int> Tokenizer::encode(const std::string& text) const {
    ...
    for (auto& tok : tokens) {
        ...
        if (it != vocab_.end())
            ...
            ids.push_back(UNK % (int)vocab_size_);
    }
    return ids;
}
...
void Tokenizer::train(const std::string& text, size_t num_merges) {
    ...
    for (size_t iter = 0; iter < num_merges && vocab_.size() < vocab_size_; ++iter) {
        ...
        for (size_t i = 0; i + 1 < words.size(); ++i) {
            // Use Unit Separator \x1F (not NUL) — NUL would truncate std::string from C-string
            // literal
...
static size_t utf8_need(const unsigned char* p, size_t avail) {
    ...
    if ((c & 0x80) == 0)
        ...
        return 1;                   // invalid lead: emit as-is (byte-level compat)
    ...
    for (size_t i = start; i < n; ++i) {
        ...
        if (need > n - i) {  // incomplete tail at i — is it a real lead?
            ...
            for (size_t k = i + 1; k < n; ++k)
                if ((((unsigned char)raw[k]) & 0xC0) != 0x80) {
                    prefix_ok = false;
                    break;
                }
...
std::vector<std::string> Tokenizer::split_pretokenize(const std::string& text) {
    ...
    while (i < n) {
        ...
        // other: run of non-space non-alnum (keeps punctuation together, splits high bytes singly?
        // no—group)
        ++j;
