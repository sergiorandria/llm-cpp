...
bool operator==(const AlignedAllocator<T, A>&, const AlignedAllocator<T, A>&) {
    return true;
}
template <typename T, size_t A>
bool operator!=(const AlignedAllocator<T, A>& a, const AlignedAllocator<T, A>& b) {
    return !(a == b);
}
...
inline bool is_aligned64(const void* p) {
